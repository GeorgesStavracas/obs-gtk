...
static void config_data_free(gpointer data)
{
...
static void obs_config_manager_finalize(GObject *object)
{
...
static void obs_config_manager_class_init(ObsConfigManagerClass *klass)
{
...
static void obs_config_manager_init(ObsConfigManager *self)
{
	for (int i = 0; i < N_SCOPES; i++) {
		self->configs[i] = g_hash_table_new_full(
			g_str_hash, g_str_equal, g_free, config_data_free);
	}
}
