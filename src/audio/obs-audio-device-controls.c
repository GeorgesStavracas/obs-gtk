...
G_DEFINE_FINAL_TYPE (ObsAudioDeviceControls, obs_audio_device_controls, ADW_TYPE_BIN)
...
static inline int
get_n_audio_channels(ObsAudioDeviceControls *self)
{
...
static const char *
get_icon_name_from_source(ObsAudioDeviceControls *self)
{
	obs_source_t *source;
	gboolean muted;
	const char *id;
...
static void
update_muted_button(ObsAudioDeviceControls *self)
{
	...
	if (obs_audio_device_get_muted(self->audio_device)) {
		gtk_widget_add_css_class(GTK_WIDGET(self->muted_button), "error");
		gtk_widget_add_css_class(GTK_WIDGET(self), "muted");
	} else {
		gtk_widget_remove_css_class(GTK_WIDGET(self->muted_button), "error");
		gtk_widget_remove_css_class(GTK_WIDGET(self), "muted");
	}
}

static void
update_widgetry(ObsAudioDeviceControls *self)
{
	obs_source_t *source;
	GtkWidget *child;
	float volume;
	...
	while ((child = gtk_widget_get_first_child(GTK_WIDGET(self->volume_meters_box))))
...
static void
recalculate_peaks(ObsAudioDeviceControls *self,
                  int64_t                 now_us)
{
	GtkWidget *child;
	gboolean is_idle;
	...
	obs_audio_peak_tracker_tick(&self->peak_tracker, now_us, ellapsed_time_us);

	is_idle = obs_audio_peak_tracker_is_idle(&self->peak_tracker, now_us);
	if (is_idle)
		obs_audio_peak_tracker_reset(&self->peak_tracker);

	n_channels = channels = obs_volmeter_get_nr_channels(self->volume_meter);
	...
	for (int i = 0; i < n_channels; i++) {
		...
		obs_audio_peak_tracker_get_levels(&self->peak_tracker,
						  actual_i,
						  &magnitude,
						  &value,
						  NULL);
...
static void
recreate_volume_ticks(ObsAudioDeviceControls *self)
{
	GtkWidget *child;

	while ((child = gtk_widget_get_first_child(GTK_WIDGET(self->volume_ticks_box))))
...
static void
on_muted_changed_cb(ObsAudioDevice         *audio_device,
                    GParamSpec             *pspec,
...
static void
on_locked_button_active_changed_cb(GtkToggleButton        *lock_button,
                                   GParamSpec             *pspec,
                                   ObsAudioDeviceControls *self)
{
	if (gtk_toggle_button_get_active(lock_button)) {
		gtk_button_set_icon_name(GTK_BUTTON(lock_button), "system-lock-screen-symbolic");
		gtk_widget_add_css_class(GTK_WIDGET(lock_button), "warning");
	} else {
		gtk_button_set_icon_name(GTK_BUTTON(lock_button), "changes-allow-symbolic");
		gtk_widget_remove_css_class(GTK_WIDGET(lock_button), "warning");
	}
}

static void
volume_meter_updated_cb(gpointer    data,
...
static gboolean
frame_clock_tick_cb(GtkWidget     *widget,
...
static void
obs_audio_device_controls_finalize (GObject *object)
{
	ObsAudioDeviceControls *self = (ObsAudioDeviceControls *)object;

	obs_audio_peak_tracker_clear(&self->peak_tracker);

	obs_volmeter_remove_callback(self->volume_meter, volume_meter_updated_cb, self);
...
static void
obs_audio_device_controls_get_property (GObject    *object,
                                        guint       prop_id,
                                        GValue     *value,
                                        GParamSpec *pspec)
{
...
static void
obs_audio_device_controls_set_property (GObject      *object,
                                        guint         prop_id,
                                        const GValue *value,
                                        GParamSpec   *pspec)
{
	...
	switch (prop_id) {
	case PROP_AUDIO_DEVICE:
		obs_audio_device_controls_set_audio_device(self, g_value_get_object(value));
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
	}
...
obs_audio_device_controls_class_init (ObsAudioDeviceControlsClass *klass)
{
	...
	properties[PROP_AUDIO_DEVICE] =
		g_param_spec_object("audio-device", NULL, NULL,
				    OBS_TYPE_AUDIO_DEVICE,
				    G_PARAM_READWRITE |
				    G_PARAM_CONSTRUCT |
	...
	gtk_widget_class_set_template_from_resource(widget_class, "/com/obsproject/Studio/GTK4/ui/audio/obs-audio-device-controls.ui");

	gtk_widget_class_bind_template_child(widget_class, ObsAudioDeviceControls, muted_button);
	gtk_widget_class_bind_template_child(widget_class, ObsAudioDeviceControls, volume_adjustment);
	gtk_widget_class_bind_template_child(widget_class, ObsAudioDeviceControls, volume_meters_box);
	gtk_widget_class_bind_template_child(widget_class, ObsAudioDeviceControls, volume_ticks_box);

	gtk_widget_class_bind_template_callback(widget_class, on_locked_button_active_changed_cb);

	gtk_widget_class_set_css_name(widget_class, "audiocontrols");
}

static void
obs_audio_device_controls_init (ObsAudioDeviceControls *self)
{
	gtk_widget_init_template(GTK_WIDGET(self));

	self->volume_meter = obs_volmeter_create(OBS_FADER_LOG);
	obs_volmeter_add_callback (self->volume_meter, volume_meter_updated_cb, self);

	obs_audio_peak_tracker_init(&self->peak_tracker);

	obs_audio_peak_tracker_start(&self->peak_tracker,
				     23.53, -60.0, 0.3,
				     20 * 1000,
				     1 * 1000);

	gtk_widget_add_tick_callback (GTK_WIDGET(self), frame_clock_tick_cb, NULL, NULL);

	recreate_volume_ticks(self);
}

GtkWidget *
obs_audio_device_controls_new (ObsAudioDevice *audio_device)
{
	return g_object_new (OBS_TYPE_AUDIO_DEVICE_CONTROLS,
...
void
obs_audio_device_controls_set_audio_device (ObsAudioDeviceControls *self,
                                            ObsAudioDevice         *audio_device)
{
	...
	if (self->audio_device) {
		...
		g_clear_signal_handler(&self->muted_changed_id, self->audio_device);
	...
	if (audio_device) {
		obs_source_t *source = obs_audio_device_get_source(audio_device);

		self->volume_binding = g_object_bind_property(audio_device,
							      "volume",
							      self->volume_adjustment,
							      "value",
							      G_BINDING_BIDIRECTIONAL | G_BINDING_SYNC_CREATE);

		self->muted_binding = g_object_bind_property(audio_device,
							      "muted",
							      self->muted_button,
							      "active",
							      G_BINDING_BIDIRECTIONAL | G_BINDING_SYNC_CREATE);

		self->muted_changed_id = g_signal_connect(audio_device, "notify::muted",
							  G_CALLBACK(on_muted_changed_cb),
							  self);
