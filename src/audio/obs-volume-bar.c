...
get_adjusted_value(ObsVolumeBar *self)
{
...
static inline double
get_adjusted_magnitude(ObsVolumeBar *self)
{
	const double value = CLAMP(self->magnitude, self->minimum, self->maximum);
	return (value - self->minimum) / (self->maximum - self->minimum);
}

static void
obs_volume_bar_measure(GtkWidget      *widget,
                       GtkOrientation  orientation,
                       int             for_size,
                       int            *minimum,
                       int            *natural,
...
static void
obs_volume_bar_size_allocate(GtkWidget *widget,
                             int        width,
                             int        height,
                             int        baseline)
{
	...
	case GTK_ORIENTATION_VERTICAL:
		gtk_widget_measure(self->magnitude_gadget,
				   ...
				   &min_magnitude_size, NULL,
				   NULL, NULL);
		allocation.x = 0;
		allocation.y = (1.0 - adjusted_magnitude) * height - min_magnitude_size / 2.0;
	...
	case GTK_ORIENTATION_HORIZONTAL:
		gtk_widget_measure(self->magnitude_gadget,
				   ...
				   &min_magnitude_size, NULL,
				   NULL, NULL);
		allocation.x = (1.0 - adjusted_magnitude) * width - min_magnitude_size / 2.0;
		allocation.y = 0;
...
static void
obs_volume_bar_dispose (GObject *object)
{
...
static void
obs_volume_bar_get_property (GObject    *object,
                               guint       prop_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
...
static void
obs_volume_bar_set_property (GObject      *object,
                             guint         prop_id,
                             const GValue *value,
                             GParamSpec   *pspec)
{
	...
	case PROP_ORIENTATION:
		...
		gtk_orientable_set_orientation(GTK_ORIENTABLE(self->level_bar), self->orientation);
		break;
	case PROP_VALUE:
...
static void
obs_volume_bar_class_init (ObsVolumeBarClass *klass)
{
...
static void
obs_volume_bar_init (ObsVolumeBar *self)
{
	...
	self->level_bar = g_object_new(GTK_TYPE_LEVEL_BAR,
				       "min-value", 0.0,
				       "max-value", 100.0,
				       "mode", GTK_LEVEL_BAR_MODE_CONTINUOUS,
				       "inverted", TRUE,
	...
	gtk_level_bar_add_offset_value(GTK_LEVEL_BAR(self->level_bar), "silent", 16.0);
	gtk_level_bar_add_offset_value(GTK_LEVEL_BAR(self->level_bar), "nominal", 66.0);
	gtk_level_bar_add_offset_value(GTK_LEVEL_BAR(self->level_bar), "warning", 85.0);
	gtk_level_bar_add_offset_value(GTK_LEVEL_BAR(self->level_bar), "error", 97.0);
	gtk_level_bar_add_offset_value(GTK_LEVEL_BAR(self->level_bar), "clip", 100.0);
	gtk_widget_set_parent(self->level_bar, GTK_WIDGET(self));

	self->magnitude_gadget = g_object_new(ADW_TYPE_BIN,
					      "css-name", "magnitude",
					      NULL);
...
GtkWidget *
obs_volume_bar_new (void)
{
	return g_object_new (OBS_TYPE_VOLUME_BAR, NULL);
}

double
obs_volume_bar_get_value (ObsVolumeBar *self)
{
...
void
obs_volume_bar_set_value (ObsVolumeBar *self,
                          double        value)
{
	...
	self->value = value;
	gtk_level_bar_set_value(GTK_LEVEL_BAR(self->level_bar), get_adjusted_value(self));

	g_object_notify_by_pspec(G_OBJECT(self), properties[PROP_VALUE]);
}

void
obs_volume_bar_get_range (ObsVolumeBar *self,
                          double       *out_minimum,
                          double       *out_maximum)
{
	g_return_if_fail(OBS_IS_VOLUME_BAR (self));
...
void
obs_volume_bar_set_range (ObsVolumeBar *self,
                          double        minimum,
                          double        maximum)
{
	g_return_if_fail(OBS_IS_VOLUME_BAR (self));
...
double
obs_volume_bar_get_magnitude (ObsVolumeBar *self)
{
	g_return_val_if_fail (OBS_IS_VOLUME_BAR (self), 0.0);
...
void
obs_volume_bar_set_magnitude (ObsVolumeBar *self,
                              double        magnitude)
{
	g_return_if_fail(OBS_IS_VOLUME_BAR(self));
