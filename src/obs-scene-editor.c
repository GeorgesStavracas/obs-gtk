...
static void draw_cb(gpointer user_data, uint32_t width, uint32_t height)
{
	obs_render_main_texture();
}

static void obs_scene_editor_dispose(GObject *object)
{
...
static void obs_scene_editor_class_init(ObsSceneEditorClass *klass)
{
	...
	gtk_widget_class_set_template_from_resource(
		widget_class,
		"/com/obsproject/Studio/GTK4/ui/obs-scene-editor.ui");

	gtk_widget_class_bind_template_child(widget_class, ObsSceneEditor,
					     display_widget);
	gtk_widget_class_bind_template_child(widget_class, ObsSceneEditor,
					     main_box);
...
static void obs_scene_editor_init(ObsSceneEditor *self)
{
	gtk_widget_init_template(GTK_WIDGET(self));

	obs_display_widget_add_draw_callback(self->display_widget, draw_cb,
					     self);
}
