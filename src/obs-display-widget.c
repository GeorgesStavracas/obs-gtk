...
static void on_display_renderer_notify_cb(GtkMediaStream *stream,
					  GParamSpec *pspec, GtkVideo *self)
{
...
static void obs_display_widget_realize(GtkWidget *widget)
{
	...
	gtk_media_stream_realize(GTK_MEDIA_STREAM(self->display_renderer),
				 surface);
}

static void obs_display_widget_unrealize(GtkWidget *widget)
{
	ObsDisplayWidget *self = OBS_DISPLAY_WIDGET(widget);
	GdkSurface *surface;
	...
	gtk_media_stream_unrealize(GTK_MEDIA_STREAM(self->display_renderer),
				   surface);

	GTK_WIDGET_CLASS(obs_display_widget_parent_class)->unrealize(widget);
}

static void obs_display_widget_map(GtkWidget *widget)
{
	ObsDisplayWidget *self = OBS_DISPLAY_WIDGET(widget);

	GTK_WIDGET_CLASS(obs_display_widget_parent_class)->map(widget);

	if (gtk_media_stream_is_prepared(
		    GTK_MEDIA_STREAM(self->display_renderer)))
		gtk_media_stream_play(GTK_MEDIA_STREAM(self->display_renderer));
}

static void obs_display_widget_hide(GtkWidget *widget)
{
	ObsDisplayWidget *self = OBS_DISPLAY_WIDGET(widget);
...
static void obs_display_widget_dispose(GObject *object)
{
...
static void obs_display_widget_class_init(ObsDisplayWidgetClass *klass)
{
...
static void obs_display_widget_init(ObsDisplayWidget *self)
{
	...
	g_signal_connect(self->display_renderer, "notify::prepared",
			 G_CALLBACK(on_display_renderer_notify_cb), self);
